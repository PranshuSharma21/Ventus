...
function checkEchoOrInvalid(text, lastQuestion = "") {
  ...
  if (lastQuestion) {
    ...
    if ((matches / words.length) > 0.35) return true;
  }
  ...
  return !hasTech && words.length < 10;
}
...
STRICT RULES:
1. If candidate echoes/parroted question, reject it and re-demand an original explanation.
2. Keep responses concise (under 3 sentences).`
      ...
      return res.json({ reply: initialQuestion, done: false });
    }
    ...
    const isInvalid = checkEchoOrInvalid(message, session.lastQuestion);
    ...
    return res.json({ reply: aiReply, done: false });
